    ...
    for r in prestes_retirados:
        if r['ean'] == ean and r['setor'] == setor:
            ...
            if r.get('validade'):
                ...
                # não houver um produto com data diferente? Por segurança,
                # vamos manter como estava, mas o script de correção vai arrumar
                return True
    return False
        ...
        for p in produtos:
            if p.ean == ean:
                adicionar_prestes_retirados(
                    ...
                    p.mes_ano  # Agora salva a validade
                )
                break
...
def api_eans_precificacao():
    """Retorna lista de EANs para precificação"""
    setor = session.get('setor_atual')
    if not setor:
        return jsonify({'erro': 'Setor não selecionado'}), 400
