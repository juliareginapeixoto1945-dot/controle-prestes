...
def adicionar_prestes_retirados(ean: str, setor: str, descricao: str, data_retirada: str, validade: str = None):
...
def verificar_produto_retirado(ean: str, setor: str, validade: str = None) -> bool:
    """
    ...
    for r in retirados:
        if r['ean'] == ean and r['setor'] == setor:
            ...
            if r.get('validade'):
                ...
                # Registro antigo sem validade - considera retirado apenas se
                # não houver um registro mais específico
                # Por segurança, vamos considerar retirado
                return True
    return False

def listar_prestes_ativos(setor: str, produtos: list, faltas: list):
    """
    ...
    for p in produtos:
        ...
        if verificar_produto_retirado(p.ean, setor, p.mes_ano):
            continue
